"""One-shot terminal review for a stage-10 next-experiment proposal."""
...
class NextExperimentTerminalSession:
    def __init__(
        self,
        ...
        *,
        ...
        self.proposal_path = proposal_path.expanduser()
        self.experiment_id = f"next-{session_id}"
        self.report_id = self.experiment_id
        self.folder = checked_root(self.settings, "next-experiment-sessions") / session_id
        self.service = NextExperimentTakeoverService(solver)
        self.phase = "startup"
        self.state = {
            "format": "next_experiment_terminal_session_v1",
    ...
    def run(self) -> int:
        if self.folder.exists() or self.folder.is_symlink():
            raise ValueError("Next-experiment session already exists; choose a new session ID")
        ...
        write_json(
            self.folder / "session.json",
            {
                ...
                "mode": "next-experiment",
                "proposal_path": str(self.proposal_path.absolute()),
                "experiment_id": self.experiment_id,
                "report_id": self.report_id,
            },
        )
        try:
            self.phase = "proposal_review"
            self.checkpoint("validating_proposal")
            self.say("步骤 1/3：重新复核来源实验、候选计划和全部场景运行条件。")
            try:
                ...
                self.say(f"候选拒绝：{exc}")
                return self.finish(
                    "proposal_rejected", 2, error_type=type(exc).__name__, reason=str(exc)
                )

            write_json(
                self.folder / "request.json", request.model_dump(mode="json")
            )
            write_json(
                self.folder / "precheck.json", request.precheck.model_dump(mode="json")
            )
            review = {
                "format": "next_experiment_execution_review_v1",
            ...
            self.say(
                f"来源实验={request.source_experiment_id}；来源重放散列="
                f"{request.source_report_sha256}"
            )
            self.say(
                f"solver 预算：计划 {request.plan.planned_calls} 次 / 上限 "
                f"{request.plan.max_calls} 次；需要 "
                f"{request.precheck.planned_external_wait_sec:g} 秒 / 总等待预算 "
                f"{request.plan.external_wait_budget_sec:g} 秒；失败策略 "
                f"{request.plan.failure_policy}。"
            )
            for case in request.plan.cases:
                decision = next(
                    item.decision for item in request.precheck.cases
            ...
            self.say(
                f"候选散列={request.proposal_sha256}\n"
                f"候选文件散列={request.proposal_file_sha256}\n"
            ...
            self.say(
                "步骤 2/3：候选文件或其中的“执行”文字不构成确认；"
                "这里只接受下方一次性确认短语。"
            )
            confirmation = f"执行下一轮实验 {review_sha256[:12]}"
            answer = self.io.read(
            ...
            report_dir = (
                checked_root(self.settings, "experiment-reports") / request.report_id
            )
            for attempt in receipt.batch.attempts:
                self.say(f"运行 {attempt.run_id}：{attempt.status}")
            verified = sum(
                summary.verified_candidates for summary in receipt.report.summaries
            )
            self.say(
                f"实际调用 {receipt.batch.calls_invoked}/{request.plan.max_calls} 次；"
            ...
            successful = (
                verified == request.plan.planned_calls
            ...
            return self.finish(
                ...
                batch_status=receipt.batch.status,
