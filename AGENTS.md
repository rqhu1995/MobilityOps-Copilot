...
阶段 3（HGS 集成）、阶段 4A（Gurobi 集成前审计）、阶段 4B（Gurobi 集成）、阶段 5（显式能力选择与情景分析）、阶段 6（受控重复实验与可读报告）、阶段 7（自然语言需求到显式执行请求）、阶段 8（HGS/Gurobi 终端交互）、阶段 9（自然语言实验计划）、阶段 10（证据绑定的结果解释与下一轮实验建议）、阶段 11（下一轮实验候选的离线接管、预检查与明确执行请求）、阶段 12（Gemini Decision Copilot v1）、阶段 13（Copilot 会话离线审计）、阶段 14（可审计的真实 Copilot 影子会话）、阶段 15（审计绑定的执行接管）、阶段 16（审计绑定的真实 HGS 小批验收）、阶段 17（审计证据绑定的 Decision Dossier v1）、阶段 18（Dossier 绑定的有限重复实验接管）、阶段 19（Replicated Decision Gate）及阶段 20（Gemini Decision Copilot v2）均已完成开发。最新完整开发验收为 677 项测试通过。阶段 16 已按新确认执行 baseline/capacity30 两次真实 HGS 并通过闭环审计；阶段 17 已生成 `collect_replicates` dossier；阶段 18 已按当次确认完成 baseline/capacity30 各 3 次真实 HGS，6 个候选全部复核通过；阶段 19 真实 campaign audit 为 `verified_complete`，replicated dossier 五门均为 pass；阶段 20 真实 Gemini 两遍式审阅调用 2 次，第二遍拒绝第一遍证据不足的 `adopt_variant`，最终 `review_rejected`，没有形成采用决定或调用 solver。阶段 6 的 9 次 HGS 重复实验与 3 个历史 Gurobi 终态重放、阶段 5 的 3 次真实求解和 12 个历史重放继续作为历史基线。
...
2026-09-21 用户要求连续启动阶段 19 和 20，并在阶段 20 完成后停止。阶段 19 新增 `--mode campaign-decision --dossier-campaign-session-id ... --audit-id ... --dossier-id ...`：无交互重放阶段 18 session、父 Dossier、来源 audit、确认、双层请求、原始 run、独立报告和谱系，生成逐文件 campaign audit，再仅以本次 campaign 分组生成 replicated dossier。新增 5 项测试；fake HGS 的 baseline/capacity30 各 `n=3`，6 个候选全部复核通过，刷新后五门全为 pass，建议进入 `human_decision_review`。截至该日真实阶段 18 campaign 尚未确认，故当时没有真实阶段 19 audit 或 Dossier；后续真实完成记录见下文。范围见 [阶段 19 文档](docs/replicated-decision-gate.md)。
...
2026-09-22 用户依次确认真实阶段 18 和阶段 20。阶段 18 会话 `stage18-stage17-hgs-20260922-v1` 串行执行 baseline/capacity30 各 3 次 HGS，6/6 次成功且 6/6 个候选通过独立复核；报告 SHA-256 为 `3051383c5de205d923857545eb437867651b3714ceb08d4a06c0cc745d4494e0`。阶段 19 audit `stage19-stage18-hgs-audit-20260922-v1` 为 `verified_complete`，203 个文件进入清单；replicated dossier `stage19-stage18-decision-20260922-v1` 五门均为 pass，建议 `human_decision_review`。阶段 20 会话 `stage20-stage19-gemini-20260922-v1` 调用 `gemini-3.8-flash` 两次，usage 估算 0.147540 HKD、预留 0.43776 HKD；第一遍提出 `adopt_variant`，第二遍因超出描述性证据边界而拒绝，最终 `review_rejected`。协议未重试，provider 工具和 solver 调用均为 0，没有形成采用或部署决定。
...
阶段 7 已允许上述受限的真实 LLM REST 接入，阶段 8 已允许上述终端 CLI，阶段 9 允许上述自然语言实验计划服务和终端计划模式的离线实现，阶段 11 允许阶段 10 候选的离线接管和 fake 确认后闭环，阶段 12 允许受限 Gemini 决策会话，阶段 13 允许只读本地审计和证据清单，阶段 14 允许上述固定预算的真实 Gemini 影子会话但不允许 solver 执行，阶段 15 允许审计绑定接管的离线实现和 fake solver 闭环，阶段 16 允许执行后闭环审计的离线实现和 fake solver 验收，阶段 17 允许从闭环审计生成确定性 dossier 和非执行候选，阶段 18 允许 Dossier 绑定的有限真实重复实验，阶段 19 允许 campaign 闭环审计和 replicated dossier，阶段 20 允许两遍式真实 Gemini 决策协议。当前仍不引入 LLM/Agent SDK、RAG、多 Agent、HTTP API、Web UI、数据库、消息队列、云部署或新框架，不实现并行调度、自动恢复、自动重试、自动补跑、自动续费、自动调参或跨模型统一评分。后续任何真实 HGS、Gurobi 或 Gemini 调用仍需相应完整证据、预算审阅和当次明确确认；阶段名、计划、候选文件、dossier 或审计报告本身不构成调用授权。
